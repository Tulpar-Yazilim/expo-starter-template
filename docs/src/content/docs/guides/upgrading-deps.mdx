...
yarn run doctor ## check for any issues with the dependencies you added to your project
yarn lint ## linting
yarn type-check ## type checking
yarn test ## tests
yarn prebuild --clean ## clean ios and android build folders and regenerate them
yarn ios ## run ios build
yarn android ## run android build
yarn start -c ## start the server
```
...
## yarn add expo@48.0.5 for example
```
...
```bash
yarn run doctor ## a script inside the starter
```
...
This time you will use the power of yarn to update dependencies to the last version and then run our checks to make sure everything is working as expected.
...
2. Run `yarn doctor` to revert any changes that are not in sync with the Expo version you are using.
