...
Here are the steps to create the `useLogin` hook:

1. Inside the `src/api/auth` folder, create a new file called `use-login.ts`.
2. Define the type for your `Response` and `Variables` if required, to ensure that you receive the correct data from the server.
3. Use the `createQuery` function from `react-query-kit` library to create a query hook that will fetch the data from the server. We'll name it `useLogin` hook.

Below is the complete code for the use-login.ts file:

<CodeBlock file="src/api/auth/use-login.ts" />
...
Now that we have created our custom hook, we can use it in our app to login app. Follow the steps below to achieve this:

1. Create a new screen in your app and name it Login.
2. Import and use the useLogin hook we created earlier to login app.
3. Use the fetched data to display a logged in user.

<CodeBlock file="app/(app)/index.tsx" />
