...
```json
{
  "scripts": {
    ...
    "version": "yarn prebuild && git add ."
  }
}
```

If you run `yarn app-release`, it will ask you to choose the type of release (major, minor, patch). based on the type of release, it will update the version inside `package.json`.
...
So technically, we just need to run `yarn app-release` and it will handle the rest for us. from updating native app version to pushing a new tag to GitHub.
...
To setup the release process for your app, you need to follow the steps from our [EAS instructions file](https://github.com/Tulpar-Yazilim/expo-starter-template/blob/master/EAS.md).
