...
As the name suggest, this hook will run after each merge and it will check if there is any changed in yarn-lock.yaml and if there is any, it will run `yarn install` to make sure the dependencies are up to date.
